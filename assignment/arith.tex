...
Now, recall that a prime number is an integer that has exactly two divisors.
\begin{exercise}
...
We recall that the set of integers is a unique factorization domain (UFD): any non-zero integer can be written
uniquely as a product $n = s p_0^{k_0} p_1^{k_1} \ldots p_{m}k^m$, $s$ is a unit, $k_i > 0$ and $p_i$ are prime
numbers.
\begin{exercise}
Write a function \texttt{factor(n)} that returns the factorisation of \texttt{n} as a list of pairs
